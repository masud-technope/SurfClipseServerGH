...
public class BingTester {
	...
	HashMap<String, ArrayList<String>> bingSolution;
	public BingTester()
	{
	
		this.orginalSolution=new HashMap<>();
		this.bingSolution=new HashMap<>();
	}
	...
	protected void collect_google_solutions()
	{
		...
		try
		{
			String path=StaticData.Lucene_Data_Base+"/completeds/browser/Bing";
			File f=new File(path);
			if(f.isDirectory())
			{
				File[] files=f.listFiles();
				for(File f1:files)
				{
					...
					scanner.close();
					//now add the arraylist to hmap
					this.bingSolution.put(f1.getName(), temp);
					//System.out.println(f1.getName()+"="+temp.size());
					}catch(Exception exc){
	...
	protected void calculate_mean_avg_precision()
	{
		double sum=0;
		int answer_found=0;
		int querysize=this.bingSolution.keySet().size();
		for(String key:this.bingSolution.keySet())
		{
			//System.out.println(key);
			ArrayList<String> glist=this.bingSolution.get(key);
	...
	public static void main(String[] args) {
		...
		BingTester tester=new BingTester();
