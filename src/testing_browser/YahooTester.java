...
public class YahooTester {
	...
	HashMap<String, ArrayList<String>> yahooSolution;
	public YahooTester()
	{
		this.orginalSolution=new HashMap<>();
		this.yahooSolution=new HashMap<>();
	}
	...
	protected void collect_google_solutions()
	{
		...
		try
		{
			String path=StaticData.Lucene_Data_Base+"/completeds/browser/Yahoo";
			File f=new File(path);
			if(f.isDirectory())
			{
				File[] files=f.listFiles();
				for(File f1:files)
				{
					...
					scanner.close();
					//now add the arraylist to hmap
					this.yahooSolution.put(f1.getName(), temp);
					//System.out.println(f1.getName()+"="+temp.size());
					}catch(Exception exc){
	...
	protected void calculate_mean_avg_precision()
	{
		double sum=0;
		int answer_found=0;
		int querysize=this.yahooSolution.keySet().size();
		for(String key:this.yahooSolution.keySet())
		{
			//System.out.println(key);
			ArrayList<String> glist=this.yahooSolution.get(key);
	...
	public static void main(String[] args) {
		...
		YahooTester tester=new YahooTester();
