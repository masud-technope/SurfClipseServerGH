...
public class SeaHawkTester {
	...
	protected void collect_google_solutions()
	{
		...
		try
		{
			String path=StaticData.Lucene_Data_Base+"/completeds/existing/seahawk";
			File f=new File(path);
			if(f.isDirectory())
			{
				File[] files=f.listFiles();
				for(File f1:files)
				{
					...
					scanner.close();
					//now add the arraylist to hmap
					this.seahawkSolution.put(f1.getName(), temp);
					//System.out.println(f1.getName()+"="+temp.size());
					}catch(Exception exc){
	...
	protected double calculate_precision_at_k(int K, ArrayList<String> googleList, ArrayList<String> solutionList)
	{
		...
		for (int i = 0; i < indexLimit; i++) {
			...
			String _link=link.split("\\s+")[0].trim();
	...
	protected void calculate_mean_avg_precision(int K)
	{
		double sum=0;
		int answer_found=0;
		int querysize=this.seahawkSolution.keySet().size();
		for(String key:this.seahawkSolution.keySet())
		{
			//System.out.println(key);
			ArrayList<String> glist=this.seahawkSolution.get(key);
	...
	public static void main(String[] args) {
		...
		SeaHawkTester tester=new SeaHawkTester();
